...
const Navbar = () => {
    ...
    const handleNavigation = (type) => {
        setMediaType(type); 
        navigate(`/${type}`); // Navigate dynamically to the type
    };
    ...
    return (
        <header className='fixed top-0 w-full h-16 bg-black bg-opacity-70 z-40'>
            <div className='container mx-auto px-5 flex items-center h-full'>
                ...
                <Link 
                    to='/' 
                    className='text-2xl lg:text-3xl font-extrabold px-2 bg-gradient-to-r from-teal-400 to-green-400 inline-block text-transparent bg-clip-text font-logoFont cursor-pointer active:scale-95 transition-all tracking-wide'
                >
                    moviemap
                </Link>
                ...
                <nav className='hidden lg:flex items-center gap-1 ml-5 text-white'>
                    <button 
                        className='px-2 cursor-pointer active:scale-90 transition-all text-white'
