...
const Carousel = () => {
  ...
  return (
    <div className='relative w-full overflow-hidden'>
      ...
      {/* Navigation Buttons */}
      <button
        className='absolute top-1/2 left-4 transform -translate-y-1/2 bg-black bg-opacity-50 text-sm md:text-base text-white w-10 h-10 hidden md:flex items-center justify-center rounded-full shadow-lg hover:bg-opacity-75 transition'
        onClick={prevSlide}
      >
        &#10094;
      </button>
      <button
        className='absolute top-1/2 right-4 transform -translate-y-1/2 bg-black bg-opacity-50 text-sm md:text-base text-white w-10 h-10 hidden md:flex items-center justify-center rounded-full shadow-lg hover:bg-opacity-75 transition'
        onClick={nextSlide}
      >
        &#10095;
      </button>
      ...
      <div className='absolute bottom-1 md:bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-1 md:space-x-2'>
        {media.map((_, index) => (
          <button
            key={index}
            className={`w-1 md:w-2 h-1 md:h-2 rounded-full ${
